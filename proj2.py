...
def she_says_he_says(she_says):
    ...
    phonetic_she_says = she_says[0:4] + 'i' + she_says[5:]         # vase reseni
    compact = phonetic_she_says[0:2] + phonetic_she_says[3:]       # vase reseni
    he_says = compact[::-1]                                        # vase reseni                                       # vase reseni
    return he_says
...
def solfege(title_hymn):
    ...
    title_hymn = 'Ut queant laxis re sonare fibris mi ra gestorum fa muli tuorum sol ve polluti la bii reatum Sancte Iohannes'

    # the input string partitioned to the title (if given) and the actual hymn
    possible_title, sep, hymn = title_hymn.split(": ")[0], ': ', title_hymn.split(": ")[len(title_hymn.split(": ")) - 1]    # vase reseni
    ...
    skip2 = [hymn_list[i] for i in range(0, len(hymn_list) - 3, 3)]            # vase reseni
